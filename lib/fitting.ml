...
module Make(Shtream : S.AnyShtream) = struct
  ...
  let shtream_apply f s =
    try f s BEFORE Shtream.add_cleanup {| Shtream.close s |} it
  ...
  let dup_protect dups thunk =
    ...
    let cleaner   = {| List.iter close_gen saved |} in
  ...
  let rec run_low = {
    s_s = (fun fitting s -> match fitting with
      | Source r   -> r.source_witness $
                        with_dups [ 0 %<* r.source_gen ] {|
                          Shtream.of_channel stdin
                        |}
      | Sink r     -> with_dups [ 1 %>* r.sink_gen ] {|
                        r.sink_witness {
                      ...
                      Shtream.nil ()
      | Trans f    -> f s
      | Ext r      -> r.ext_in_witness ^$ Shtream.of_thunk {|
                        r.ext_out_witness { out_witness = fun out_w ->
                          let s' = out_w s in
                          with_dups [ 0 %</ Shtream.channel_of s' ] {|
                            r.ext_c_c ()
                          |}
                        }
                      |}
      | Status n   -> Shtream.from {| Shtream.fail_with n |}
      | DupIn r    -> dup_protect (r.dupin_dups :> dup_spec) {|
                        run_low.s_s r.dupin_sub
                          (if has_descr Unix.stdin r.dupin_dups
                           then r.dupin_witness (Shtream.of_channel stdin)
                           else s)
                      |}
      | DupOut r   -> dup_protect (r.dupout_dups :> dup_spec) {|
      ...
      | Seq r      -> 
          ...
          let snd = ref None in
            Shtream.from_low
              ~close:{| Shtream.close (maybe !snd {| s1 |} id) |}
              {| maybe !snd
                 {| maybe (Shtream.next' s1)
                    {| let status = maybe (Shtream.status s1)
                                      {| Proc.WEXITED 0|} id in
    ...
    s_c = (fun fitting s out_w -> match fitting with
      | Source r   -> with_dups [ 0 %<* r.source_gen ]
                        copy_in_out
      | Sink r     -> with_dups [ 1 %>* r.sink_gen ] {|
                        r.sink_witness {
                          out_witness = fun out_w ->
                      ...
                      |}
      | Trans f    -> out_w { out_witness = fun out_w ->
                        Shtream.output (out_w (f s))
                      }
      | Ext r      -> r.ext_out_witness { out_witness = fun out_w ->
                        let s' = out_w s in
                        with_dups [ 0 %</ Shtream.channel_of s' ] {|
                          r.ext_c_c ()
                        |}
      ...
      | DupIn r    -> with_dups (r.dupin_dups :> dup_spec) {|
                        run_low.s_c r.dupin_sub
                          ...
                          out_w
                      |}
      | DupOut r   -> with_dups (r.dupout_dups :> dup_spec) {|
    ...
    c_c = (fun fitting in_w out_w -> match fitting with
     ...
     p_s_s = {
       ...
       | Source s1, t2 ->
           dup_protect [ 0 %<* s1.source_gen ] {|
             shtream_apply
               (run_low.s_s t2)
               (s1.source_witness (Shtream.of_channel stdin))
           |}
       | t1, Sink s2 ->
           with_dups [ 1 %>* s2.sink_gen ] {|
     ...
     p_c_c = {
       ...
       | t1, t2 ->
           ...
           unwind_protect {|
             let f s = 
               shtream_apply (run_low.s_s t2) (run_low.s_s t1 s)
                 BEFORE (to_close := Some it) in
             run_low.c_c (Trans f) in_w out_w
           |}
           {| maybe !to_close ignore Shtream.close |}
     };
  }
  ...
  let command cmd =
    make_ext {| Proc.exec cmd |}

  let program ?path prog ?argv0 args =
    make_ext {| Proc.exec_program ?path prog ?argv0 args |}

  let thunk next =
    make_ext {| Abort.abort next |}
